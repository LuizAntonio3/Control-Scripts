...
def phi_koop(v):
    return np.array([
        np.sin(v[0]),        # 1st Harmonic Sine (x1)
        np.cos(v[0]),        # 1st Harmonic Cosine (x1)
        np.sin(2*v[0]),      # 2nd Harmonic Sine (x1)
        np.cos(2*v[0]),      # 2nd Harmonic Cosine (x1)
        np.sin(3*v[0]),      # 3rd Harmonic Sine (x1)
        np.cos(3*v[0]),      # 3rd Harmonic Cosine (x1)
        np.sin(v[1]),        # 1st Harmonic Sine (x2)
        np.cos(v[1]),        # 1st Harmonic Cosine (x2)
        np.sin(2*v[1]),      # 2nd Harmonic Sine (x2)
        np.cos(2*v[1]),      # 2nd Harmonic Cosine (x2)
        np.sin(3*v[1]),      # 3rd Harmonic Sine (x2)
        np.cos(3*v[1])       # 3rd Harmonic Cosine (x2)
    ], dtype=float)
...
def plot_G():
    plt.figure(dpi=150, constrained_layout=True, figsize=(8, 4))
    plt.subplot(1, 2, 1)
    ...
    red_line = mlines.Line2D([], [], color='red', linewidth=2, label='NN (Cycle)')
    plt.legend(handles=[blue_line, red_line], loc='lower right', fontsize=8)
...
def model(t: float, x: np.ndarray[float], nx: int, G: ntx.Graph, mi: float, E_half: list, _progress=[0.0]):
    ...
    for k in cycle_edges:
        ...
        delta_x = x_hat_real_all[j] - x_hat_real_all[i]
    ...
    gamma_lr = 3.0 if t > 0.5 else 0.0
    C_max = 5.0   # safe bound: true sin(v) coefficient is ~2.0, so this is well above it
    sigma = 2.0   # sigma modification gain
    continuous_leak_gain = 0.1 # continuous leakage to suppress noise on irrelevant basis functions
    ...
    for k in cycle_edges:
        ...
        C_koop_dot[k] = gamma_lr * (e[i] - e[j]) * phi_k / norm_factor - sigma_mod * C_koop[k] - continuous_leak_gain * C_koop[k]
...
for m in range(len(t)):
    ...
    for k in cycle_edges:
        ...
        i_hat_start = i * nx[0] + N * nx[0]
        j_hat_start = j * nx[0] + N * nx[0]
        
        z_hat_i = x[i_hat_start:i_hat_start+nx[0], m]
        z_hat_j = x[j_hat_start:j_hat_start+nx[0], m]
        
        x_hat_i_real = np.array([z_hat_i[0], z_hat_i[1] * (z_hat_i[0]**2 + 100) / 10])
        x_hat_j_real = np.array([z_hat_j[0], z_hat_j[1] * (z_hat_j[0]**2 + 100) / 10])
        
        delta_x = x_hat_j_real - x_hat_i_real
