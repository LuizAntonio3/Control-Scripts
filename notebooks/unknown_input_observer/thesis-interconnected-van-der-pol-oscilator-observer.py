...
# --- RBF Network Setup ---
K_rbf = 25
c_rbf = np.linspace(-10, 10, K_rbf)
sigma_rbf = (20.0 / (K_rbf - 1)) * 1.2

def phi_rbf(v):
    return np.exp(-0.5 * ((v - c_rbf) / sigma_rbf)**2)

n_edges = len(E_half)
x0_weights = np.zeros(n_edges * K_rbf)

x0 = np.concat([x0_systems, x0_observer, x0_levants, x0_weights], axis=0)
...
def model(t: float, x: np.ndarray[float], nx: int, G: ntx.Graph, mi: float, E_half: list, _progress=[0.0]):
    if t - _progress[0] >= 1.0:
        print(f"Simulating t={t:.1f}s")
        _progress[0] = t
    ...
    # --- DUIO RBF Network Update (Hybrid) ---
    n_edges = len(E_half)
    W_start_id = 2*N*nx + N*nlevants
    W = x[W_start_id:].reshape(n_edges, K_rbf)
    ...
    for k in cycle_edges:
        ...
        phi_k = phi_rbf(delta_x)
        phi_edges_k_map[k] = phi_k
        
        g_hat_ij = np.dot(W[k], phi_k)
        
        d_NN_all[i] += g_hat_ij
        d_NN_all[j] -= g_hat_ij
        
    e = np.clip(D_res - d_NN_all, -20.0, 20.0)
    gamma_lr = 20.0 if t > 0.5 else 0.0
    
    for k in cycle_edges:
        edge = E_half[k]
        i = edge[0] - 1
        j = edge[1] - 1
        # Add Weight Decay (L2 Regularization) to fix the cycle null-space drift
        weight_decay = 0.5 * W[k] if t > 0.5 else 0.0
        W_dot[k] = gamma_lr * (e[i] - e[j]) * phi_edges_k_map[k] - weight_decay
        
    xdot[W_start_id:] = np.expand_dims(W_dot.flatten(), axis=1)
...
# %%
# %%
# Reconstruct edge signals from the neural network weights
W_start_id = 2*N*nx[0] + N*nlevants
n_edges = len(E_half)
...
for m in range(len(t)):
    ...
    for k in cycle_edges:
        ...
        W_k = x[W_start_id + k*K_rbf : W_start_id + (k+1)*K_rbf, m]
        phi_k = phi_rbf(delta_x)
        g_hat_all[k, m] = np.dot(W_k, phi_k)
